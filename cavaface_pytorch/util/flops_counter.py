...
def count_model_flops(model, input_res=[112, 112], multiply_adds=True, device=torch.device('cpu')):
    ...
    model.eval()
    foo(model)
    input = Variable(torch.rand(3,input_res[1],input_res[0]).unsqueeze(0), requires_grad = True).to(device)
    out = model(input)
